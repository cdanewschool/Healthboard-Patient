...
[Bindable] public var howToHandleMessageTabs:String = "not created";		//howToHandleMessageTabs is a fix; if we call createNewMessage from the appointments module (i.e. sending a msg to a doctor or nurse) or viewMessage from the Widgets module before the messages module has been created, then it messes up everything... so, the first time we want to createNewMessage from appts or viewMessage from Widgets, we just set it to "viewWidgetMessage" or "createApptsMessage", and the corresponding function will be called from the messages module instead...
...
protected function bar_initializeHandler():void {
	...
	if(howToHandleMessageTabs == "viewWidgetMessage") {
		...
		var recipient:uint = appointmentsModule.selectedAppointment.type == "appointment" ? 1 : 2;
...
public function openMessage(event:ListEvent):void {
	this.currentState = 'modMessages';
	if(howToHandleMessageTabs != "not created") {		//howToHandleMessageTabs is a fix; if we call createNewMessage from the appointments module (i.e. sending a msg to a doctor or nurse) or viewMessage from the Widgets module before the messages module has been created, then it messes up everything... so, the first time we want to createNewMessage from appts or viewMessage from Widgets, we just set it to "viewWidgetMessage" or "createApptsMessage", and the corresponding function will be called from the messages module instead...
		viewMessage(event.itemRenderer.data);
		tabsMessages.selectedIndex = viewStackMessages.length - 2;
	}
	else {
		myMsgToOpen = event.itemRenderer.data;
		howToHandleMessageTabs = "viewWidgetMessage";
	}
}